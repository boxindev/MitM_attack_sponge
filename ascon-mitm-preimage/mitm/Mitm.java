...
public class Mitm {
  ...
  public Mitm(final GRBEnv env, final int Rounds) throws GRBException {
    ...
    GRBLinExpr DoF_red_count = new GRBLinExpr();
    GRBLinExpr DoF_blue = new GRBLinExpr();
    GRBLinExpr DoF_blue_count = new GRBLinExpr();
    GRBLinExpr DoM = new GRBLinExpr();
    
    GRBLinExpr Consume_blue = new GRBLinExpr();
    ...
    for (int i = 0; i < 32; i++)
      for (int j = 1; j < 5; j++)  {
        DoF_red.addTerm(-1.0, DA[0][i][j][0]);
        DoF_red.addConstant(1.0);
        DoF_blue.addTerm(-1.0, DA[0][i][j][2]);
        DoF_blue.addConstant(1.0);

        DoF_red_count.addTerm(-1.0, DA[0][i][j][0]);
        DoF_red_count.addConstant(1.0);
        DoF_blue_count.addTerm(-1.0, DA[0][i][j][2]);
        DoF_blue_count.addConstant(1.0);
      }
    ...
    for (int round = 0; round < Rounds; round ++)
      for (int i = 0; i < 32; i++)
        for (int k = 0; k < 5; k++) {
          DoF_red.addTerm(-1.0, DC[round][i][k][0]);
        }

    for (int round = 0; round < Rounds; round ++)
      for (int i = 0; i < 32; i++)
        for (int k = 0; k < 5; k++) {
          DoF_blue.addTerm(-1.0, DC[round][i][k][1]);
          Consume_blue.addTerm(1.0, DC[round][i][k][1]);
        }
    ...
    for (int i = 0; i < 32; i++) {
      ...
      DoF_red_count.addTerm(1.0, dom[i]);
      DoF_blue_count.addTerm(1.0, dom[i]);
    }
    ...
    model.addConstr(DoF_red_count, GRB.LESS_EQUAL, 96, "");
    model.addConstr(DoF_blue_count, GRB.LESS_EQUAL, 96, "");
    //model.addConstr(Consume_blue, GRB.EQUAL, 0, "");
    ...
    try {
      ...
      for (int round = 0; round < Rounds; round++)
        for (int i = 0; i < 32; i++)
          for (int j = 0; j < 5; j++)
            for (int k = 0; k < 2; k++){
              DCValue[round][i][j][k]  = (int) Math.round(DC[round][i][j][k].get(GRB.DoubleAttr.Xn));
              for (int l = 0; l < 3; l++)  {
