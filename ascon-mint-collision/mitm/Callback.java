...
public class Callback extends GRBCallback {
    ...
    private MitmSolution getSolution(int solutionNumber, double obj, double[] x) {
        ...
        for (int round = 0; round < Rounds; round++)
            for (int i = 0; i < 32; i++)
                for (int j = 0; j < 5; j++)  {
                    DCValue[round][i][j]  = (int) Math.round(x[960*Rounds+544+5*(32*round+i)+j]);
                }


        for (int i = 0; i < 32; i++) {
            DomValue[i]  = (int) Math.round(x[1120*Rounds+544+i]);
        }

        int[] dcmatchValue  = new int[32];
        for (int i = 0; i < 32; i++) {
            dcmatchValue[i]  = (int) Math.round(x[1120*Rounds+544+32+i]);
