...
@Command(name = "mitmsearch", mixinStandardHelpOptions = true, version = "1.0", description = "Find MIMT attacks")
public class MitmSearch implements Callable<Integer> {
  ...
  @Option(names = {"-nSols","-sol"}, defaultValue = "1", description = "Maximum number to find. Default is ${DEFAULT-VALUE}")
  private int nSols;
  ...
  public Integer call() {
    ...
    String output = "output/result_"+Rounds+".json";
  ...
  private List<MitmSolution> getMitmSolutions() {
    try {
      ...
      //if (verbose)
        //System.out.println("Starting: initila round "+initr + "; match round "+matr+".");
      List<MitmSolution> MitmSolutions = mitm.solve(nSols, nonOptimalSols, MinObjectiveValue, Threads);
      if (MitmSolutions.get(0).objective>0) {
